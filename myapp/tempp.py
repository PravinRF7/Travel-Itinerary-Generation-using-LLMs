...
class ModelSingleton:
    ...
    def get_instance(cls):
        if cls._instance is None:
            cls._instance = ModelInitializer()
...
class ModelInitializer:
    def __init__(self):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(self.model_name_or_path, device_map="auto", torch_dtype=torch.float16)
    ...
    def get_output(self, input_text):
        ...
        tokens = self.tokenizer(prompt_template, return_tensors='pt').input_ids.to(DEVICE)
...
def get_iternary(place_name, days, min_price, max_price):
    try:
        # Model Initialization using Singleton
        model_initializer = ModelSingleton.get_instance()
        ...
        return itinerary, summary, urls, start_date, end_date
